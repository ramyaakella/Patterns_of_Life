...
let width = 600,
  ...
  maxRadius = 3;
let curr_minute = 0;
let modes = ["AtWork", "AtRecreation", "AtHome", "AtRestaurant", "Transport"];

let angle = (2 * Math.PI) / (modes.length - 1);

let center_mode = "Transport",
  center_pos = { x: 300, y: 365 };
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // const schedule = await d3.json("../data/schedule.json");
  ...
  // nodes for each person's schedule
  ...
  const tempSimulation = d3
    ...
    .force("center", d3.forceCenter(width / 2, height / 2).strength(0)) // disable the attraction force
    .force("charge", d3.forceManyBody().strength(0)) // disable the repulsion force
    .force(
      "collide",
